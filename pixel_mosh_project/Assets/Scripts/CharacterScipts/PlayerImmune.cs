...
public class PlayerImmune : PlayerState 
{
   private float _ImmuneDelay;
   ...
   private Rigidbody2D _rb;
    ...
    public override void UpdateState(){
        if(_TimeCounter > _ImmuneDelay){
            player.Effect = new Queue<float>();
            player.State = new PlayerAlive(player);
        }
        IntereactWeapon();
        AimGun();
        ShootWeapon();
